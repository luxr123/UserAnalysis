 ...
 * .....
 * JedisUtil jedisCache = JedisUtil.getInstance(props);
 * 
 * @author jason.hua
 *
 */
public class JedisUtil {
	...
	private static final Logger logger = LoggerFactory.getLogger(JedisUtil.class);
	 ...
	 private static JedisUtil jcache = null;
...
//  	 private ShardedJedisPool shardedJedisPool = null;
	 ...
	 private JedisUtil(Properties props) {  
		 init(props);
	 }
	   ...
	   private void init(Properties props) { 
		    if (jedisPool == null) { 
	    	    ...
	    	    jedisPool = new JedisPool(config, 
	    	    		props.getProperty("redis.master.ip"),  
	    	            Integer.valueOf(props.getProperty("redis.master.port")),
	    	            10000,
	    	            props.getProperty("redis.master.password"));  
		     }
	   }
	     ...
	     * @return
	     */
	    public static JedisUtil getInstance(Properties props) {
	    	if(jcache == null){
	    		synchronized(JedisUtil.class){
	    			if(jcache == null){
		    			jcache = new JedisUtil(props);
	    			}
	    		}
	    	}
			return jcache;
		...
		public static void main(String[] args) throws IOException {
			Properties props = new Properties();
			props.put("redis.pool.maxActive", "1024");
			props.put("redis.pool.maxIdle", "200");
			props.put("redis.pool.maxWait", "1000");
			props.put("redis.pool.testOnBorrow", "true");
			props.put("redis.pool.testOnReturn", "true");
			props.put("redis.master.ip", "10.100.2.94");
			props.put("redis.master.port", "6379");
			props.put("redis.master.password", "123456");
			JedisUtil jedisCache = JedisUtil.getInstance(props);
			String key="TopIpData";
			double score=1.0;
			logger.info("Add 1 million ip data to Redis...");
			for(long i=200000;i>0;i--){
				String member="10.100.0."+i;
				jedisCache.SORTSET.zincrby(key, score+i, member);
				if(i%1000==0){
					logger.info(i+"data inserted.");
				}
			}
